...
// PILLAR_WEIGHTS (40/35/15/5/5). Glow basket + Top Picks tabs re-score
// composites whenever the user changes these.
const PILLAR_WEIGHTS_KEY = "glow-screener-pillar-weights-v1";
...
// Weight Lab — a sandbox pillar-weight mix, independent of the live Glow
// weights, so the desk can experiment (and let AI search) without touching
// the published basket until they hit "Apply to Glow".
const LAB_WEIGHTS_KEY = "glow-screener-lab-weights-v1";
...
const TECH_PARAMS_KEY = "glow-screener-tech-params-v1";
...
const ALLOC_KEY = "glow-screener-alloc-per-pick-v1";
...
// side our Glow top 7. Keyed by month "YYYY-MM" (the month the cohort
// is HELD, not the month-end entry date — June 2026 = "2026-06"; the
// entry close comes from the last May snapshot).
const CLIENT_COHORT_KEY = "glow-screener-client-cohort-v1";
...
//               for the whole new month (the Glow production model)
...
const COHORT_VIEW_KEY = "glow-screener-cohort-view-v1";
...
// + per-pick accuracy.
const STRATEGY_MODE_KEY = "glow-screener-strategy-mode-v1";
...
const ACTIVE_CADENCE_KEY = "glow-screener-active-cadence-v1";
...
const MANUAL_RETURN_KEY = "glow-screener-manual-return-mode-v1";
...
const STRATEGY_SUBTAB_KEY = "glow-screener-strategy-subtab-v1";
...
const SIM_PREFS_KEY = "glow-screener-sim-prefs-v1";
const SIM_DEFAULTS = {
...
const HISTORY_VIEW_KEY = "glow-screener-history-view-v1";
...
// in the Glow basket pillar editor, the History tab still shows the
...
const RECOMPUTE_HISTORY_KEY = "glow-screener-recompute-history-v1";
...
const WATCHLIST_KEY = "glow-screener-watchlist-v1";
...
async function switchTab(tabId) {
  ...
  $$(".tab-btn").forEach((b) => {
    // Top Picks has no nav tab of its own — it's launched from the Glow
    // (composite) tab, so keep Glow highlighted while viewing it.
  ...
  // "★ Top Picks" launcher button lives in the controls row, Glow tab only.
...
async function loadTab(tabId) {
  ...
  // Composite (Glow Basket) tab: fuse fundamentals + technicals + macro
  ...
  if (c.composite) {
    ...
    // once extraction lands for a ticker — routine processes ~10/day in Glow order).
...
function renderStats() {
  ...
  if (c.composite && st) {
    ...
    $("#stat-rules-note").textContent = `In Glow Basket of ${st.scored.length}`;
    ...
    $("#top-cards-title").innerHTML = `<span class="text-amber-500">🏆</span> Glow Basket — Top 10 Picks <span class="ml-2 text-xs font-normal text-slate-500">composite-weighted, hard-fails excluded</span>`;
    return;
  }
...
// Glow Basket — premium overview: a rating-distribution strip
...
function renderCompositeTopCards() {
  ...
  // Glow basket header stays clean.
  ...
  const printCover = `
    <div class="print-only print-cover">
      <div style="padding: 30mm 10mm; text-align: center;">
        ...
        <p style="font-size: 11px; color: #94a3b8; margin-top: 36px;">For internal use only · Glow Research</p>
      </div>
    </div>
  `;
...
// Pillar Weights modal — invoked from the Glow basket header pill.
    ...
    <div class="px-7 py-6">
      <div class="flex items-start justify-between gap-4 mb-1">
        <div>
          ...
          <p class="text-sm text-slate-500 mt-1">Set the relative importance of each pillar. Composite scores update across Glow Basket and Top Picks.</p>
        </div>
...
function renderTopPicks() {
  ...
  if (picks.length === 0) {
    $("#top-picks-content").innerHTML = `
      <button id="tp-back-btn" type="button" class="mb-3 inline-flex items-center gap-1.5 text-sm font-semibold text-indigo-600 hover:text-indigo-700">← Back to Glow Basket</button>
  ...
  $("#top-picks-content").innerHTML = `
    <button id="tp-back-btn" type="button" class="mb-3 inline-flex items-center gap-1.5 text-sm font-semibold text-indigo-600 hover:text-indigo-700 print-hide">← Back to Glow Basket</button>
    ${heroHeader}
    ${picks.length > 0 ? `
...
// closes, Glow manual picks, and the out-of-coverage OHLCV injection step.
...
async function ensureHistoryCache() {
  ...
  // Inject synthetic stock entries for out-of-coverage tickers (Glow picks
  ...
  // Glow picks indexed by ticker so manual-row clicks resolve to the
...
async function renderHistory() {
  ...
  // analyst flip between History (past STRONG BUYs + Glow card) and
...
function wireCohortHandlers(seriesData) {
  ...
  // forensics). Manual rows reuse the Glow pick (client-entry framing
...
// can render. Manual rows prefer the Glow pick (with entry / targets / SL
...
function buildCohortClickPick(ticker, side, segAnchor) {
  ...
  if (side === "manual" && lkpByTicker?.has(ticker)) {
    ...
    // Only return the Glow pick if it has usable data — otherwise the
...
// Build pick objects for the Glow basket from the committed file + snapshot
...
// Glow Manual picks card — the client's hand-curated basket, tracked with the
  ...
  return `
    <div class="bg-white rounded-2xl ring-1 ring-slate-100 p-4 sm:p-5 mt-4">
      <div class="flex flex-wrap items-center justify-between gap-2 mb-1">
        <h2 class="font-display font-bold text-slate-900 text-base">Glow Manual picks</h2>
        <div class="flex items-center gap-3">
...
// Targets / stop-loss block shown inside the Glow drill modal, in place of the
...
async function handleLkpExcelUpload(file) {
  try {
    ...
    saveLkpOverride({ label: "Glow Manual picks", source: "Excel upload (browser preview)", generated_at: new Date().toISOString(), picks });
...
// At each month-end, lock the Glow basket's top 7 (highest composite,
...
// the client's Glow picks (same entry-month logic, midpoint of entry
...
// Falls back to most-recent month-end snapshot if no Glow file exists
...
function renderHistoryViewSwitch(activeView, accuracyData, cohortView) {
  ...
  const cohortToggle = `
    <div id="cohort-view-toggle" class="inline-flex bg-slate-100 rounded-lg p-0.5 text-[11px] font-semibold">
      <button data-view="static" type="button" class="${cohortBtnCls(cohortView === "static")}" title="AI top 7 frozen from prior month-end · held all month (Glow model)">Static</button>
...
// use TGT1 + SL columns from the Glow upload (per-stock).
//
...
//   manualPicks → the Glow basket (in-universe rows have ticker + tgt1 + sl).
...
function renderCohortTracker(cohort, series, view, selectedSegIdx) {
  ...
  const manualRows = series.manualPicksAll.length
    ...
    : `<div class="text-[11px] text-slate-400 text-center py-4 leading-relaxed">No client basket loaded.<br>Use the Glow picks card below to upload one.</div>`;
...
// likes on the Glow Basket page — one click, no scroll.
...
// Manual uses the client's per-stock TGT1 / SL from the Glow upload.
...
function openHistoryDrill(pick) {
  ...
  // Target / Stop-loss levels for the overlay. Glow picks carry explicit
  ...
  // Glow picks reuse this modal but with a client-entry framing: no "we said
  ...
  if (isFiltered) {
    populateHardFailRules(pick.ticker).catch(() => {
      ...
      if (el && panel && panel.dataset.ticker === pick.ticker) {
        el.textContent = "Couldn't load live rule list — open Glow Basket for the full breakdown.";
      }
    });
  }
...
// computed (via Glow tab or a prior drill-down), returns the cached
...
// drill can resolve a ticker → company object without forcing the Glow
// basket tab to be visited first.
//
// Note: prefers the enriched rows from state.cache.composite when Glow
...
async function populateHardFailRules(ticker) {
  ...
  // (auditor opinion, governance flags, revenue-mix) the Glow basket uses.
  ...
  if (!co) {
    writeIfCurrent("Couldn't resolve ticker — open Glow Basket for the full breakdown.", true);
    return;
  }
  const result = await ensureCompositeFor(co);
  if (result == null) {
    writeIfCurrent("Couldn't load live rule list — open Glow Basket for the full breakdown.", true);
    return;
  }
...
// ---------------- Glow Basket — radar / thesis / animation / magazine ----------------
...
function synthesizeThesis(s) {
  ...
  const sentences = [];
  sentences.push(`Composite scores <strong>${composite}/100</strong> — ${rating} territory per the Glow framework.`);
  if (top && top[1].pct >= 75) {
      ...
      <div class="lg:col-span-2 p-7 border-r border-slate-100">
        ...
        <div class="bg-gradient-to-br ${theme.soft} rounded-2xl ring-1 ${theme.ring} p-5">
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-[10px] text-slate-500 whitespace-nowrap">Glow · Section C</span>
          </div>
...
// Glow Basket drill — opens as a CENTRED MODAL (not the side panel).
  ...
  const hardFailPanel = s.hardFails.length ? `
    <div class="mb-6 p-5 bg-gradient-to-br from-rose-50 to-pink-50 rounded-2xl ring-1 ring-rose-200">
      <div class="flex items-start gap-3 mb-3">
        ...
        <div class="flex-1">
          <div class="font-bold text-rose-900 text-base">Excluded from Glow Basket</div>
...
function buildHelpModal(tabId) {
  ...
  // Glow Basket — composite-specific help: weights + bands + hard fails.
  if (cfgTab.composite) {
    ...
    return `
      <div class="relative bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-6 text-white">
        ...
        <div class="text-xs font-semibold uppercase tracking-wider opacity-90">Glow — Stock Selection &amp; Performance Index</div>
        <h2 class="text-2xl font-bold mt-1">How the Glow Basket Is Scored</h2>
      ...
      <div class="p-6 max-h-[calc(90vh-180px)] overflow-y-auto">
        ...
        <div>
          ...
          <div class="bg-rose-50/40 rounded-xl p-4 ring-1 ring-rose-200">
            <div class="text-xs text-rose-800 mb-3">If any of these trigger, the stock is excluded from the Glow basket regardless of its composite score:</div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ${hardFails.map((hf) => `
  ...
  return `
    <div class="relative bg-gradient-to-br ${grad} p-6 text-white">
      ...
      <p class="text-sm opacity-90 mt-1">${rules.length} rules across ${Object.keys(grouped).length} categories · max ${totalMax} pts · scored verbatim from the client's Glow framework.</p>
    </div>
    <div class="p-6 max-h-[calc(90vh-180px)] overflow-y-auto">
...
// focus if the user hasn't visited the Glow basket yet.
...
const CUSTOM_STRATS_KEY = "glow-screener-custom-strategies-v3";
const STRAT_FIELDS = [
...
// Replicates the Glow composite (a weighted blend of the 5 pillars) but lets
...
function renderWeightLab() {
  ...
  // Anchor on the CURRENT live weights (what the Glow basket actually uses),
  ...
  return `
    <section id="weight-lab" class="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 p-4 sm:p-5">
      ...
      <div class="text-sm text-slate-600 mb-4 max-w-2xl">Change how much each pillar counts and watch which basket the mix would have picked — then let AI hunt for the best mix. This is the same 5-pillar blend the Glow Basket uses; nothing changes on the live basket until you hit <strong>Apply to Glow</strong>.</div>
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div>
          ...
          <div class="flex items-center gap-2 mt-3 flex-wrap">
            ...
            <button type="button" data-lab-apply class="px-3 py-1.5 rounded-lg bg-indigo-600 text-white text-xs font-semibold hover:bg-indigo-700">Apply to Glow Basket</button>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
          ${renderLabResultCard("Current Glow", defBadge, currentW, perfCurrent, false)}
...
function wireWeightLab() {
  ...
  $(`${root} [data-lab-apply]`)?.addEventListener("click", () => {
    ...
    alert(`Applied to Glow Basket: F${w.fundamentals} · T${w.technicals} · M${w.macro} · S${w.sentiment} · L${w.liquidity}. Open the Glow Basket tab to see the re-scored basket.`);
    refreshWeightLab();   // re-render so the "Current Glow" card reflects the new live weights
  });
  $(`${root} [data-lab-reset]`)?.addEventListener("click", () => {
    // Reset = the framework default (F40/T35/M15/S5/L5), not whatever the live
    // Glow weights may have drifted to (client: "reset to default").
...
async function renderCustom() {
  ...
  try {
    ...
    // sliders always start from what the Glow basket actually uses.
...
const ALERT_PREFS_KEY = "glow-screener-alert-prefs-v1";
