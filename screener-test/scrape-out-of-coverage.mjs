#!/usr/bin/env node
// Out-of-coverage OHLCV scraper. The Glow manual basket sometimes
...
async function run() {
  if (!existsSync(LKP_PATH)) {
    console.log("No Glow manual picks file — nothing to scrape.");
    return;
  }
  ...
  // `picksByMonth`). For Glow rows where `ticker` is null because we
  ...
  if (tickers.size === 0) {
    console.log("No out-of-coverage tickers in Glow picks. Nothing to fetch.");
  ...
  const payload = {
    ...
    note: "Daily closes for Glow manual-pick stocks that fall outside our coverage universe. Used by the Performance Tracker + Accuracy view so out-of-cap picks count in the Manual average return and hit tracking.",
